...
public class StreetMaker : MonoBehaviour {
	...
	void Update () {
		...
		if ( randomNumber < 0.85f ) {
			...
			Transform rightBuilding = (Transform)Instantiate( buildingPrefab, transform.position + transform.right * 5f, transform.rotation );
			//    set that clone's localScale to (1f, [random number between 1 and 3], 1f)
			rightBuilding.localScale = new Vector3( 1f, Random.Range (1f, 10f), 1f );
		} else {
			...
			if ( anotherRandom < 0.5f ) {
				...
				Instantiate ( streetMakerPrefab, transform.position, Quaternion.Euler ( 0f, transform.localEulerAngles.y - (Random.value > 0.62f ? 45f : 90f), 0f ) );
			} else {
		...
		if ( stepCount >= 15 || Physics.Raycast ( forwardRay, 5f) ) {
